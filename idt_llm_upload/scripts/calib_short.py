...
if __name__=="__main__":
    grid=[]
    for N,k in [(23,2),(23,3),(24,2),(30,2),(39,2)]:
        for beta in (0.0,2.0):
            for rho in (0.0,0.5):
    ...
    for gi,(N,k,beta,rho,gamma,nonstat,imbal) in enumerate(grid):
        acc={n:[0,0] for n in ('global','strat','local','circ')}; epsvals=[]
        for s in range(nsim):
            r=one(N,k,beta,rho,gamma,nonstat,imbal,seed_of(gi,s))
            if r is None: continue
            epsvals.append(r.pop('_eps',np.nan))
            for n,v in r.items():
        ...
        row=dict(N=N,k=k,beta=beta,rho=rho,gamma=gamma,nonstat=nonstat,imbal=imbal,
                 ...
                 mean_eps=round(float(np.nanmean(epsvals)),4) if epsvals else np.nan)
        ...
        pd.DataFrame(rows).to_csv('/home/claude/trackB/calibration_short.csv',index=False)
    print("done")
